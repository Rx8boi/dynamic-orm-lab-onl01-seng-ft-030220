...
#Instance methods to insert data into db
  ...
  def save
    DB[:conn].execute("INSERT INTO #{table_name_for_insert}
     (#{col_names_for_insert}) VALUES (?)," [values_for_insert, grade])
  ...
  def self.find_by_name
    DB[:conn].execute("SELECT * FROM #{self.table_name} WHERE name = ?," [name, grade])
  end

  def self.find_by
    DB[:conn].execute("SELECT * FROM #{self.table_name} WHERE ? = ?")
  end
